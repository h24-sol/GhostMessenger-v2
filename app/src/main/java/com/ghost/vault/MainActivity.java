...
public class MainActivity extends AppCompatActivity {
    private String SERVICE_ID = "ghost.v2.p2p";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        chat.setText("--- GHOST P2P INITIALIZED ---\n");
        layout.addView(chat);
    ...
    private final ConnectionLifecycleCallback connectionCallback = new ConnectionLifecycleCallback() {
        @Override public void onConnectionInitiated(String id, ConnectionInfo info) { client.acceptConnection(id, payloadCallback); }
        @Override public void onConnectionResult(String id, ConnectionResolution res) { if (res.getStatus().isSuccess()) remoteId = id; }
    ...
    private final PayloadCallback payloadCallback = new PayloadCallback() {
        @Override public void onPayloadReceived(String id, Payload p) {
            String data = new String(p.asBytes(), StandardCharsets.UTF_8);
            chat.append("\n[PEER]: " + data);
        }
        @Override public void onPayloadTransferUpdate(String id, PayloadTransferUpdate u) {}
    };
}
